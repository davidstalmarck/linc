...
def rolling_mvo_backtest(
    ...
    # We treat each row as a "step." If data is daily or hourly, adjust accordingly.
    all_dates = returns_wide.index
    symbols = returns_wide.columns
    w_prev = pd.Series(index=symbols, data=1.0 / len(symbols))  # equal weights
    ...
    for i in range(window_size, len(returns_wide) - 1):
        ...
        try:
            ...
            # If it's infeasible or fails, skip
            w_opt = w_prev
            print(f"Error: {e}")
        ...
        # portfolio return = sum of (w_opt * returns_next)
        port_ret = np.dot(w_opt.values, returns_next.values)

        print(np.isnan(port_ret))
        w_prev = w_opt
        # update capital
...
def evaluate_backtest(equity_df: pd.DataFrame):
    ...
    """
    # compute daily (or per-step) returns from equity
    ...
    ann_factor = 365 * 24  # if daily; if hourly, adjust
    if ret_series.std() != 0:
...
def main():
    ...
    # plt.plot(all_weights["STOCK1"], label="STOCK1")
    # plt.plot(all_weights["STOCK2"], label="STOCK2")
    # plt.plot(all_weights["STOCK3"], label="STOCK3")
